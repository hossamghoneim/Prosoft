...
let KTDatatable = function () {
        ...
        datatable = $("#kt_datatable").DataTable({
            ...
            ajax: {
                data: function () {
                    ...
                    datatable.DataTable().ajax.url(`/about-us-middle-section-items?page=${info.page + 1}&per_page=${info.length}`);
                }
            },
            columns: [
                { data: 'id' },
                ...
                { data: 'description' },
                { data: 'order' },
                { data: 'about_us_middle_section.title' },
                { data: 'is_active' },
                { data: 'created_at' },
                { data: null },
            ],
                        ...
                        return `
                            ...
                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                                ...
                                <div class="menu-item px-3">
                                    <a href="/about-us-middle-section-items/${row.id}/edit" class="menu-link px-3 d-flex justify-content-between edit-row" >
                                ...
                                <!--begin::Menu item-->
                                <div class="menu-item px-3">
                                    <a href="/about-us-middle-section-items/${row.id}" class="menu-link px-3 d-flex justify-content-between" >
                                ...
                                <!--begin::Menu item-->
                                <div class="menu-item px-3">
                                    <a href="#" class="menu-link px-3 d-flex justify-content-between delete-row" data-row-id="${row.id}" data-type="${translate('about us middle section item')}">
            ...
            deleteAlert(type).then(function (result) {
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        ...
                        url: '/about-us-middle-section-items/' + rowId,
                        success: () => {
    ...
    let handleAddButtonClick = function() {
        $(document).on('click', 'a[href*="about-us-middle-section-items/create"]', function(e) {
            e.preventDefault();

            $.ajax({
                url: '/about-us-middle-section-items/check-exists',
                method: 'GET',
                success: function(response) {
                    if (response.exists) {
                        errorAlert(response.message);
                    } else {
                        window.location.href = '/about-us-middle-section-items/create';
                    }
                },
                error: function() {
    ...
    let updateAddButtonState = function() {
        $.ajax({
            ...
            error: function() {
                console.error('Error checking about us middle section item availability');
            }
        });
    }
    ...
    let enableAddButton = function() {
        ...
        const enabledLink = toolbar.querySelector('a[href*="about-us-middle-section-items/create"]');
        if (!enabledLink) {
            const addLink = document.createElement('a');
            addLink.href = '/about-us-middle-section-items/create';
    ...
    let disableAddButton = function() {
        ...
        if (!disabledButton) {
            ...
            addButton.setAttribute('title', 'You cannot add more items because all about us middle sections have reached the limit of 3 items');
            addButton.innerHTML = `
                <span class="svg-icon svg-icon-2">
                    <i class="fa fa-plus fa-lg"></i>
                </span>
