...
let KTDatatable = function () {
        ...
        datatable = $("#kt_datatable").DataTable({
            ...
            ajax: {
                data: function () {
                    ...
                    datatable.DataTable().ajax.url(`/contact-us-sections?page=${info.page + 1}&per_page=${info.length}`);
                }
            },
            columns: [
                { data: 'id' },
                    ...
                    render: function (data, type, row) {
                        ...
                        <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                            <div class="menu-item px-3">
                                <a href="/contact-us-sections/${row.id}/edit" class="menu-link px-3 d-flex justify-content-between edit-row">
                                    <span>${translate('Edit')}</span><span><i class="fa fa-edit text-primary"></i></span>
                                </a>
                            </div>
                            <div class="menu-item px-3">
                                <a href="/contact-us-sections/${row.id}" class="menu-link px-3 d-flex justify-content-between">
                                    <span>${translate('Show')}</span><span><i class="fa fa-eye text-black-50"></i></span>
                                </a>
                            </div>
                            <div class="menu-item px-3">
                                <a href="#" class="menu-link px-3 d-flex justify-content-between delete-row" data-row-id="${row.id}" data-type="${translate('contact us section')}">
    ...
    let handleAddButtonClick = function() {
        $(document).on('click', 'a[href*="contact-us-sections/create"]', function(e) {
            e.preventDefault();
            $.ajax({
                url: '/contact-us-sections/check-exists',
                method: 'GET',
                success: function(response) {
                    if (response.exists) {
                        errorAlert(response.message);
                    } else {
                        window.location.href = '/contact-us-sections/create';
                    }
                },
                error: function() {
    ...
    let updateAddButtonState = function() {
        $.ajax({
            ...
            error: function() {
                console.error('Error checking contact us section availability');
            }
        });
    }
    ...
    let enableAddButton = function() {
        ...
        const enabledLink = toolbar.querySelector('a[href*="contact-us-sections/create"]');
        if (!enabledLink) {
            const addLink = document.createElement('a');
            addLink.href = '/contact-us-sections/create';
            addLink.className = 'btn btn-primary';
            addLink.setAttribute('data-bs-toggle', 'tooltip');
            addLink.setAttribute('title', '');
            addLink.innerHTML = `<span class="svg-icon svg-icon-2"><i class="fa fa-plus fa-lg"></i></span>Add new section`;
    ...
    let disableAddButton = function() {
        ...
        if (!disabledButton) {
            ...
            addButton.setAttribute('title', 'You cannot add more sections because all contact us contents have reached the limit of 3 sections');
            addButton.innerHTML = `<span class="svg-icon svg-icon-2"><i class="fa fa-plus fa-lg"></i></span>Add new section`;
