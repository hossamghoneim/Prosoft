...
let KTDatatable = function () {
    ...
    let handleAddButtonClick = function() {
        $(document).on('click', 'a[href*="partner-banner-section-items/create"]', function(e) {
            e.preventDefault();

            $.ajax({
                url: '/partner-banner-section-items/check-exists',
                method: 'GET',
                success: function(response) {
                    if (response.exists) {
                        errorAlert(response.message);
                    } else {
                        window.location.href = '/partner-banner-section-items/create';
                    }
                },
                error: function() {
    ...
    let updateAddButtonState = function() {
        $.ajax({
            ...
            error: function() {
                console.error('Error checking partner banner section item availability');
            }
        });
    }
    ...
    let enableAddButton = function() {
        ...
        const enabledLink = toolbar.querySelector('a[href*="partner-banner-section-items/create"]');
        if (!enabledLink) {
            const addLink = document.createElement('a');
            addLink.href = '/partner-banner-section-items/create';
    ...
    let disableAddButton = function() {
        ...
        if (!disabledButton) {
            ...
            addButton.setAttribute('title', 'You cannot add more items because all partner banner sections have reached the limit of 3 items');
            addButton.innerHTML = `
                <span class="svg-icon svg-icon-2">
                    <i class="fa fa-plus fa-lg"></i>
                </span>
